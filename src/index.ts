...
public dataExtensionCheck(soapInstance : any, oauthToken :any, dataExtensionName : any) {
  let soapMessage =
        '<?xml version="1.0" encoding="UTF-8"?>' +
        '<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:a="http://schemas.xmlsoap.org/ws/2004/08/addressing" xmlns:u="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd">' +
        "    <s:Header>" +
  ...
  this.getRefreshTokenHelper(refreshToken, tssd, false, res)
    .then((response) => {
      ...
      let DCmsg =
        ...
        "                <CategoryID>" +
        /**Data extension's folder ID */ +
        "</CategoryID>" +
        "                <CustomerKey>" + /**Data Extension's External key */+
        "</CustomerKey>" +
        "                <Name>" + /**Data Extension's name */+
        "</Name>" +
        "                <Fields>" + 
        "                    <Field>" + //Different Field type
        "                        <CustomerKey>"+/***FIELD ID***/+"</CustomerKey>" +
